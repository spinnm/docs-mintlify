...
<CardGroup cols={2}>
  ...
  <Card title="Authentication" icon="key">
    **Static header auth** via Bearer token or API key – or **no authentication** for open or network-internally secured servers. OAuth flows are not supported.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="Choose authentication">
    ...
    - **No authentication** → no header is sent and no field is filled in. Only for open or network-internally secured servers.
