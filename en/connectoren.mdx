...
Domain-system and MCP connectors support up to three authentication methods. Which ones are available is defined by the respective connector type.
...
| **None (NoAuth)** | – | No credentials are requested and **no auth header** is sent. For open or network-internally secured systems. Currently only selectable for the **[MCP server](/en/mcp-server)** connector. |
